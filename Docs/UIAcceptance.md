...
- 面板固定宽 320 pt、最大高 700 pt；屏幕较矮时高度自动收缩，只有中间任务列表滚动。
- Header、筛选区、Footer 固定；卡片高 104 pt、间距 6 pt，一屏约 4–5 张。
...
| 场景 | 结果 |
| --- | --- |
| `normal` | 320 pt 窄面板、四种状态、筛选计数与固定 Footer 正常 |
| `empty` | 高度自动收紧，空态居中，无多余大面积留白 |
| `loading` | 连接中状态、进度指示和辅助文字清晰 |
| `error` | 错误横幅与重试入口可见，不遮挡任务列表 |
| `long` | 标题、路径、分支、模型均单行截断，卡片高度不变 |
| `many` | 64 条任务可滚动到底，Header 与 Footer 始终固定 |

辅助功能树可识别筛选按钮、状态、任务摘要、开关和“在 Codex 中打开”动作。Reduce Transparency 与完整键盘/VoiceOver 操作仍应在发布签名前做一次人工系统设置验收。
